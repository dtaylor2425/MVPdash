...
def latest() -> Dict[str, Any]:
    ...
    try:
        with get_connection() as conn:
            out = store.fetch_latest(conn, universe, cfg)
...
def history(
    ...
    try:
        with get_connection() as conn:
            out = store.fetch_history(conn, t, days)
    except Exception as e:
        if _is_missing_table(e):
            raise HTTPException(status_code=404, detail="No options flow published yet")
        raise _db_unavailable(e)
...
def ticker_detail(ticker: str, include_trades: bool = Query(default=True)) -> Dict[str, Any]:
    universe, cfg = load_universe(), load_config()
    t = _valid_ticker(ticker, universe)
    try:
        with get_connection() as conn:
            out = store.fetch_ticker(conn, t, universe, cfg, include_trades=include_trades)
    except Exception as e:
        if _is_missing_table(e):
            raise HTTPException(status_code=404, detail="No options flow published yet")
        raise _db_unavailable(e)
