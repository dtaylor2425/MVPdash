    ...
    with ha:
        with st.container(border=True):
            ...
            if isinstance(comps, dict) and comps:
                # Only show real pillars — exclude backward-compat alias keys
                # which have weight=0.0 and contribution=0.0
                REAL_PILLARS = {
                    "growth_momentum", "inflation_price", "monetary_policy",
                    "market_internals", "fiscal_external", "sentiment",
                    # v4 fallback — shown only if v5 pillars not present
                    "credit", "real_yields", "curve", "risk_appetite",
                    "dollar", "cpi_momentum",
                }
                # Prefer v5 pillar keys; if none exist fall back to v4 keys
                has_v5 = any(k in comps for k in [
                    "growth_momentum","inflation_price","monetary_policy"])
                show_keys = (
                    {"growth_momentum","inflation_price","monetary_policy",
                     "market_internals","fiscal_external","sentiment"}
                ...
                rows = sorted(
                    [{"name": c.get("name", k)[:22],
                      "contrib": float(_cc(c)),
                      "z": float(c.get("zscore") or 0)}
                     for k, c in comps.items()
                     if isinstance(c, dict) and k in show_keys
                     and (c.get("weight", 0) or 0) > 0],
