...
# Only render real pillars — skip backward-compat alias keys (weight=0)
_V5_PILLARS = {"growth_momentum","inflation_price","monetary_policy",
               "market_internals","fiscal_external","sentiment"}
_V4_PILLARS = {"credit","real_yields","curve","risk_appetite","dollar","cpi_momentum"}
_has_v5 = any(k in components for k in _V5_PILLARS)
_show   = _V5_PILLARS if _has_v5 else _V4_PILLARS

if isinstance(components, dict):
    for key, c in components.items():
        if not isinstance(c, dict): continue
        if key not in _show: continue
        if (c.get("weight") or 0) == 0: continue  # skip zero-weight aliases
        level    = c.get("level")
        z        = c.get("zscore")
    ...
    with st.container(border=True):
        ...
        def _comp_interp(key, name, z, contrib, level):
            ...
            interps = {
                ...
                "risk_appetite": {
                    "pos": f"Breadth z {z:+.2f} — equal-weight (RSP) leading cap-weight (SPY). "
                           "Genuine broad market participation, not a narrow mega-cap rally.",
                    "neg": f"Breadth z {z:+.2f} — equal-weight lagging cap-weight. "
                           "Rally is narrowing — historically precedes broader market weakness.",
                },
                "dollar": {
        ...
        if not comp_df.empty and isinstance(components, dict):
            ...
            for key, c in components.items():
                if not isinstance(c, dict): continue
                if key not in _show: continue               # skip aliases
                if (c.get("weight") or 0) == 0: continue   # skip zero-weight
                result = _comp_interp(
